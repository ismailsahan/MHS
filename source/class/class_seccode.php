...
class seccode {
	...
	var $code;//验证码
	var $type 	= 0;//0 英文图片验证码  1 中文图片验证码  2 Flash验证码  3 语音验证码  4 位图验证码
	var $width 	= 150;//验证码图片宽度
	var $height 	= 60;//验证码图片高度
	var $background	= 1;//随机图片背景
	var $adulterate	= 1;//随机背景图形
	var $ttf 	= 0;//随机 TTF 字体
	var $angle 	= 0;//随机倾斜度
	var $warping 	= 0;//随机扭曲
	var $scatter	= 0;//图片打散级别
	var $color 	= 1;//随机颜色
	var $size 	= 0;//随机大小
	var $shadow 	= 1;//文字阴影
	var $animator 	= 0;//GIF 动画
	var $seccodelength = 4;//验证码长度
	var $fontpath	= '';//字体路径
	var $datapath	= '';//验证码数据路径
	var $includepath= '';//类路径
	...
	function adulteratefont() {
		...
		$x = $this->width / $this->seccodelength;
		$y = $this->height / 10;
	...
	function ttffont() {
		...
		$seccodelength = $this->seccodelength;
	...
	function flash() {
		$spacing = 5;
		$codewidth = ($this->width - $spacing * 5) / $this->seccodelength;
		$strforswdaction = '';
		for($i = 0; $i <= 3; $i++) {
