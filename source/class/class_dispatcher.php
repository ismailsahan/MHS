...
class Dispatcher {
	...
	public static function dispatch($config = array()) {
		...
		if(!defined('IN_UC') && !empty($config['siteurl']) && !striexists($config['siteurl'], '127.0.0.1') && !striexists($config['siteurl'], 'localhost') && !striexists($_G['siteurl'], $config['siteurl'])) {
	...
	public static function generate($url='', $vars='', $suffix=true, $redirect=false, $domain=false) {
		...
		if(isset($vars['referer']) && empty($vars['referer'])) $vars['referer']=urlencode(empty($_G['referer']) ? $_G['currenturl'] : $_G['referer']);
