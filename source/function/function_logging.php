...
function login($username, $password='', &$errmsg='', $uid=0, $email='', $redirect=true) {
	...
	if($uid == 0/* || empty($email)*/) {	// 正常登录
		...
		if($session['activated']){
			...
			$url = empty($_G['referer']) || strexists($_G['referer'], 'logging') ? U('main/index') : $_G['referer'];
		}else{
...
function adduser($uid, $user=array(), $profile=array()){
	...
	DB::query('UPDATE %t SET `uid`=%d WHERE `studentid`=%s', array('manhours', $uid, $profile['studentid']));
	return true;
}
