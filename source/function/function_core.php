 ...
 * 
 * 版本 v0.1.0
 * 最后修改 2013-11-29 20:57 by gwc0721
 */
 ...
 * @param string $operation 操作模式(ENCODE|DECODE) 默认为DECODE，表示解密，ENCODE表示加密
 * @param string $key		密钥（默认使用$_G['authkey']）
 * @param int	$expiry	有效期（单位秒，0表示永不过期）
 */
 ...
 * 
 * @param int	 $length  长度
 * @param boolean $numeric 是否生成随机数字，0表示生成随机字串，1表示随机数字
 * @return mixed
 */
...
function submitcheck($var, $allowget = 0, $seccodecheck = null, $secqaacheck = 0) {
	global $_G, $seccode_err, $secqaa_err;
	$name = $var;
	...
	//unset($_SESSION[$name.'Token']);
	//trace($session);
	if($session['time']>=TIMESTAMP-300 && $token === $session['hash']){
