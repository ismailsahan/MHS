...
class ManhourAction extends Action {
	...
	public function manage(){
		...
		if(IS_AJAX){
			...
			$manhours = DB::fetch_all(subusersqlformula(null, 'id,'.DB::table('manhours').'.uid,'.DB::table('manhours').'.status,aid,username,realname,gender,'.DB::table('manhours').'.manhour,aid,actname,time,applytime,verifytime,operator,remark,verifytext', 'manhours').' ORDER BY `applytime` DESC');

			if(!$template->isCached('manhour_manage')){
				$template->assign('sidebarMenu', defaultNav());
				$template->assign('adminNav', adminNav());
				$template->assign('menuset', array('mhour', 'manage'));
			}

			$template->assign('manhours', $manhours, true);
			$template->display('manhour_manage');
		}
	}
