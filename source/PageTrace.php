...
function filter($val, $level = 0){
	if(is_string($val)){
		$ret = htmlentities($val, ENT_COMPAT, 'utf-8');
		...
		foreach($val as $k => $tmp){
			$ret .= '<span style="padding-left:'.($level*2).'em"></span>';
			$ret .= $k.': '.filter($tmp, $level+1);
		}*/
		if(empty($val)){
			$ret = 'Array()';
		}else{
...
#think_page_trace_tab{
	...
	animation-name:tptt;
	-webkit-animation-name:tptt;
	-moz-animation-name:tptt;
	-ms-animation-name:tptt;
	-o-animation-name:tptt;
}
</style>
<div id="think_page_trace" style="position: fixed;bottom:0;right:0;font-size:14px;width:100%;z-index: 999999;color: #000;text-align:left;font-family:'微软雅黑';">
<div id="think_page_trace_tab" style="display:none;background:white;margin:0;height:250px;">
<div id="think_page_trace_tab_tit" style="padding:6px 12px 0;border-bottom:1px solid #ececec;border-top:1px solid #ececec;font-size:16px;overflow:auto;height:30px">
...
<div id="think_page_trace_tab_cont" style="overflow:auto;height:212px;padding:0;line-height:24px">
	...
	if(is_array($info)){
		...
		echo '<li style="border-bottom:1px solid #EEE;font-size:14px;padding:0 12px">' . (is_numeric($k) ? '' : $k.' : ') . filter($val) .'</li>';
	    }
	}
    ?>
    </ol>
...
<div id="think_page_trace_open" style="height:30px;float:right;text-align:right;overflow:hidden;position:fixed;bottom:0;right:0;color:#000;line-height:30px;cursor:pointer;opacity:.5"><div style="background:#232323;color:#FFF;padding:0 6px;float:right;line-height:30px;font-size:14px">页面调试</div></div>
...
open.onclick = function(){
	...
	document.getElementsByTagName("html")[0].style.paddingBottom = "250px";
	history[0] = 1;
